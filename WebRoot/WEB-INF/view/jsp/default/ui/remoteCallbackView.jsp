...
<head>
    <script type="text/javascript">
        ...
        <c:if test="${remoteLoginMessage != null && remoteLoginMessage != ''}">
        	//alert('<spring:message code="${remoteLoginMessage}"/>');
        	errorMessage =  "&errorMessage="+ encodeURIComponent('<spring:message code="${remoteLoginMessage}"/>');
        </c:if>        
        ...
        <c:if test="${service != null && service != ''}">
       		 var tmp = encodeURIComponent('${service}');
        	 tmp = tmp.substring(0,tmp.indexOf(encodeURIComponent('&service=')));        
             service = tmp==''?'':"&service=" + encodeURIComponent(tmp);        
        </c:if>
        // 跳转回去
